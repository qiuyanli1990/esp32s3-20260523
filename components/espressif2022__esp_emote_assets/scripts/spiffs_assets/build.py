...
def process_extra_emoji_dirs(extra_emoji_dirs, assets_dir):
    """Process optional emoji directories that should be added to emoji_collection."""
    emoji_collection = []
    for emoji_dir in extra_emoji_dirs or []:
        if not emoji_dir:
            continue
        if not os.path.exists(emoji_dir):
            print(f"Warning: Extra emoji directory not found: {emoji_dir}")
            continue
        emoji_collection.extend(process_board_emoji_collection(emoji_dir, emoji_dir, assets_dir))
...
def process_board_layout(layout_json_file, assets_dir):
    ...
    try:
        ...
        return []
def process_board_collection(resolution_dir, res_path, assets_dir, extra_emoji_dirs=None):
    ...
    if os.path.exists(res_path) and os.path.exists(resolution_dir):
        ...
        emoji_collection.extend(process_extra_emoji_dirs(extra_emoji_dirs, assets_dir))
        ...
        emoji_collection = process_extra_emoji_dirs(extra_emoji_dirs, assets_dir)
        icon_collection = []
        layout_json = []
...
def main():
    ...
    parser.add_argument('--extra_emoji_dir', action='append', default=[],
                        help='Extra directory containing emote.json and .eaf files to add to emoji_collection')
    ...
    if(args.resolution):
        emoji_collection, icon_collection, layout_json = process_board_collection(
            args.resolution, args.res_path, assets_dir, args.extra_emoji_dir)
    else:
        emoji_collection = []
        icon_collection = []
        layout_json = []
