...
void WifiStation::StartConnect() {
    ...
    const bool lock_bssid = remember_bssid_ != 0;
    if (lock_bssid) {
    ...
    ESP_LOGI(TAG,
        "Connecting AP: ssid=%s bssid=%02x:%02x:%02x:%02x:%02x:%02x rssi=%d channel=%d authmode=%d lock_bssid=%d",
        ap_record.ssid.c_str(),
        ap_record.bssid[0], ap_record.bssid[1], ap_record.bssid[2],
        ap_record.bssid[3], ap_record.bssid[4], ap_record.bssid[5],
        ap_record.rssi, ap_record.channel, ap_record.authmode, lock_bssid);
