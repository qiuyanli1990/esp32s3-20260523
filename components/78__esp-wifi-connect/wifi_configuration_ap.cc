...
constexpr size_t kMaxAgentIdLength = 128;
...
void AppendAgentOption(std::vector<AgentOption>& options, const std::string& raw_id, const std::string& raw_label) {
    const std::string id = TrimString(raw_id);
    if (id.empty()) {
        return;
    }
    ...
    std::string label = TrimString(raw_label);
    if (label.empty()) {
        label = id;
    }
...
std::vector<AgentOption> GetAgentOptions(const std::string& selected_agent_id) {
    std::vector<AgentOption> options;
    if (strlen(CONFIG_SENTINO_AGENT_OPTIONS_JSON) > 0) {
        cJSON* parsed = cJSON_Parse(CONFIG_SENTINO_AGENT_OPTIONS_JSON);
        if (cJSON_IsArray(parsed)) {
            cJSON* item = nullptr;
            cJSON_ArrayForEach(item, parsed) {
                if (cJSON_IsString(item) && item->valuestring != nullptr) {
                    AppendAgentOption(options, item->valuestring, item->valuestring);
                    continue;
                }
                ...
                if (!cJSON_IsString(id_item) || id_item->valuestring == nullptr) {
                    id_item = cJSON_GetObjectItemCaseSensitive(item, "agent_id");
                }
                cJSON* label_item = cJSON_GetObjectItemCaseSensitive(item, "label");
                if (!cJSON_IsString(label_item) || label_item->valuestring == nullptr) {
                    label_item = cJSON_GetObjectItemCaseSensitive(item, "name");
                }

                const std::string id =
                    (cJSON_IsString(id_item) && id_item->valuestring != nullptr) ? id_item->valuestring : "";
                const std::string label =
                    (cJSON_IsString(label_item) && label_item->valuestring != nullptr) ? label_item->valuestring : id;
                AppendAgentOption(options, id, label);
            }
        } else if (parsed != nullptr) {
            ESP_LOGW(TAG, "CONFIG_SENTINO_AGENT_OPTIONS_JSON is not a JSON array");
        }
        if (parsed != nullptr) {
            cJSON_Delete(parsed);
        }
    ...
    const std::string default_agent_id = TrimString(CONFIG_SENTINO_DEFAULT_AGENT_ID);
...
cJSON* BuildAgentOptionsJson(const std::string& selected_agent_id) {
    cJSON* array = cJSON_CreateArray();
    if (array == nullptr) {
        return nullptr;
    }

    for (const auto& option : GetAgentOptions(selected_agent_id)) {
        cJSON* item = cJSON_CreateObject();
        if (item == nullptr) {
            continue;
        }
        cJSON_AddStringToObject(item, "id", option.id.c_str());
        cJSON_AddStringToObject(item, "label", option.label.c_str());
        cJSON_AddItemToArray(array, item);
    }
    return array;
}

bool SaveAgentIdSelection(const std::string& agent_id, std::string* active_agent_id) {
    const std::string normalized_agent_id = TrimString(agent_id);

    nvs_handle_t nvs;
    esp_err_t err = nvs_open(kWifiNamespace, NVS_READWRITE, &nvs);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to open NVS for agent_id: %s", esp_err_to_name(err));
        return false;
    }

    if (normalized_agent_id.empty()) {
        err = nvs_erase_key(nvs, kAgentIdKey);
        if (err == ESP_ERR_NVS_NOT_FOUND) {
            err = ESP_OK;
        }
    } else {
        err = nvs_set_str(nvs, kAgentIdKey, normalized_agent_id.c_str());
    }
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to save agent_id: %s", esp_err_to_name(err));
        return false;
    }

    if (active_agent_id != nullptr) {
        *active_agent_id = normalized_agent_id.empty() ? TrimString(CONFIG_SENTINO_DEFAULT_AGENT_ID) : normalized_agent_id;
    }
    return true;
}
...
WifiConfigurationAp::WifiConfigurationAp()
{
    ...
    agent_id_ = TrimString(CONFIG_SENTINO_DEFAULT_AGENT_ID);
}
...
void WifiConfigurationAp::StartAccessPoint()
{
    ...
    agent_id_ = TrimString(CONFIG_SENTINO_DEFAULT_AGENT_ID);
    nvs_handle_t nvs;
    esp_err_t err = nvs_open(kWifiNamespace, NVS_READONLY, &nvs);
    if (err == ESP_OK) {
        // 读取WiFi功率
        ...
        char agent_id[kMaxAgentIdLength] = {0};
        size_t agent_id_size = sizeof(agent_id);
        err = nvs_get_str(nvs, kAgentIdKey, agent_id, &agent_id_size);
        if (err == ESP_OK) {
            std::string stored_agent_id = TrimString(agent_id);
            if (!stored_agent_id.empty()) {
                agent_id_ = stored_agent_id;
            }
        }
...
void WifiConfigurationAp::StartWebServer()
{
    ...
    httpd_uri_t form_submit = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            cJSON *agent_id_item = cJSON_GetObjectItemCaseSensitive(json, "agent_id");
            ...
            // 获取当前对象
            auto *this_ = static_cast<WifiConfigurationAp *>(req->user_ctx);
            std::string agent_id_str = this_->agent_id_;
            if (agent_id_item != nullptr) {
                if (!cJSON_IsString(agent_id_item) || agent_id_item->valuestring == NULL) {
                    cJSON_Delete(json);
                    httpd_resp_send(req, "{\"success\":false,\"error\":\"Invalid agent ID\"}", HTTPD_RESP_USE_STRLEN);
                    return ESP_OK;
                }
                agent_id_str = TrimString(agent_id_item->valuestring);
                if (agent_id_str.length() >= kMaxAgentIdLength) {
                    cJSON_Delete(json);
                    httpd_resp_send(req, "{\"success\":false,\"error\":\"Agent ID is too long\"}", HTTPD_RESP_USE_STRLEN);
                    return ESP_OK;
                }
            }
            ...
            if (!SaveAgentIdSelection(agent_id_str, &this_->agent_id_)) {
                cJSON_Delete(json);
                httpd_resp_send(req, "{\"success\":false,\"error\":\"Failed to save agent selection\"}", HTTPD_RESP_USE_STRLEN);
                return ESP_OK;
            }
    ...
    httpd_uri_t advanced_config = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            cJSON_AddStringToObject(json, "selected_agent_id", this_->agent_id_.c_str());

            cJSON* agent_options = BuildAgentOptionsJson(this_->agent_id_);
            if (agent_options != nullptr) {
                cJSON_AddItemToObject(json, "agent_options", agent_options);
            }
    ...
    httpd_uri_t advanced_submit = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            if (agent_id != nullptr) {
                if (!cJSON_IsString(agent_id) || agent_id->valuestring == nullptr) {
                    cJSON_Delete(json);
                    nvs_close(nvs);
                    httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "Invalid agent ID");
                    return ESP_FAIL;
                }

                std::string requested_agent_id = TrimString(agent_id->valuestring);
                if (requested_agent_id.length() >= kMaxAgentIdLength) {
                    cJSON_Delete(json);
                    nvs_close(nvs);
                    httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "Agent ID is too long");
                    return ESP_FAIL;
                }
                ...
                if (requested_agent_id.empty()) {
                    ...
                    this_->agent_id_ = TrimString(CONFIG_SENTINO_DEFAULT_AGENT_ID);
                } else {
                    err = nvs_set_str(nvs, kAgentIdKey, requested_agent_id.c_str());
                    this_->agent_id_ = requested_agent_id;
                }

                if (err != ESP_OK) {
                    ESP_LOGE(TAG, "Failed to save agent_id: %d", err);
                    cJSON_Delete(json);
                    nvs_close(nvs);
                    httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "Failed to save agent ID");
                    return ESP_FAIL;
                }
            }
            ...
            ESP_LOGI(TAG, "Saved settings: agent_id=%s, max_tx_power=%d, remember_bssid=%d, sleep_mode=%d",
                this_->agent_id_.c_str(), this_->max_tx_power_,
