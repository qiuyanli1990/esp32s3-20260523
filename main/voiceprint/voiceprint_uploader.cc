...
bool CapturePcm(AudioService& audio_service, AudioCodec* codec, std::vector<int16_t>* pcm,
    ...
    *effective_audio_ms = 0;
    ...
    const bool keep_duplex_output_clock = codec->duplex() && codec->output_enabled();
    if (codec->output_enabled() && !keep_duplex_output_clock) {
        codec->EnableOutput(false);
        vTaskDelay(pdMS_TO_TICKS(100));
    } else if (keep_duplex_output_clock) {
        ESP_LOGI(kTag, "Keep output enabled during voiceprint capture to preserve duplex RX clock");
    }
...
bool VoiceprintUploader::IsEnabled() {
    return SENTINO_ENABLE_VOICEPRINT != 0;
}
