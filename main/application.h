...
#define MAIN_EVENT_AGENT_SELECTION_NEXT (1 << 10)
...
private:
    ...
    AudioCodecRealtimeAudioPort realtime_audio_port_;
    ...
    std::mutex agent_selection_mutex_;
    std::vector<AgentSelectionOption> agent_selection_options_;
    size_t agent_selection_index_ = 0;
    int64_t agent_selection_deadline_ms_ = 0;
    std::atomic<bool> agent_selection_active_{false};
    int clock_ticks_ = 0;
    ...
    bool HandleAgentSelectionToggle();
    bool SelectAgentBeforeConversationStart();
    void ShowAgentSelectionOption(const AgentSelectionOption& option, bool selected);
