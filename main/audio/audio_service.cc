...
void AudioService::Initialize(AudioCodec* codec) {
    ...
    audio_processor_->OnOutput([this](std::vector<int16_t>&& data) {
        ...
        if (callback) {
            ++processed_pcm_callback_count_;
            callback(std::vector<int16_t>(data.begin(), data.end()));
            return;
        }
        ...
        if (bits & AS_EVENT_AUDIO_TESTING_RUNNING) {
            ...
            if (ReadAudioData(data, 16000, samples)) {
                if (codec_->input_channels() > 1) {
                    ...
                    for (size_t i = 0, j = static_cast<size_t>(capture_channel); i < mono_data.size(); ++i, j += codec_->input_channels()) {
                        mono_data[i] = data[j];
                    }
...
void AudioService::HandleSpeakerReference(const std::vector<int16_t>& data) {
    ...
    if (codec_->output_sample_rate() != 16000) {
        ...
        reference.assign(data.begin(), data.end());
    }
