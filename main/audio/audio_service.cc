...
#if (CONFIG_IDF_TARGET_ESP32S3 || CONFIG_IDF_TARGET_ESP32P4) && CONFIG_USE_CUSTOM_WAKE_WORD
#include "wake_words/custom_wake_word.h"
#elif (CONFIG_IDF_TARGET_ESP32S3 || CONFIG_IDF_TARGET_ESP32P4) && CONFIG_USE_AFE_WAKE_WORD
...
constexpr UBaseType_t kAudioInputTaskPriority = 8;
constexpr UBaseType_t kAudioOutputTaskPriority = 4;
constexpr UBaseType_t kOpusCodecTaskPriority = 2;
...
AudioService::~AudioService() {
    ...
    if (speaker_reference_resampler_ != nullptr) {
        esp_ae_rate_cvt_close(speaker_reference_resampler_);
    }
}
...
void AudioService::Initialize(AudioCodec* codec) {
...
#endif
    codec_->SetOutputTap([this](const std::vector<int16_t>& data) {
        HandleSpeakerReference(data);
    });
    ...
    audio_processor_->OnOutput([this](std::vector<int16_t>&& data) {
        std::function<void(std::vector<int16_t>&& data)> callback;
        {
            ...
            callback = processed_pcm_callback_;
        }
        if (callback) {
            callback(std::move(data));
            return;
        }
        PushTaskToEncodeQueue(std::move(data));
    });
...
void AudioService::Start() {
    ...
    xTaskCreatePinnedToCore([](void* arg) {
        ...
        vTaskDelete(NULL);
    }, "audio_input", 2048 * 3, this, kAudioInputTaskPriority, &audio_input_task_handle_, 0);
    ESP_LOGI(TAG, "Audio input task created: priority=%u core=0",
        static_cast<unsigned>(kAudioInputTaskPriority));
    ...
    xTaskCreate([](void* arg) {
        ...
        audio_service->AudioInputTask();
        vTaskDelete(NULL);
    }, "audio_input", 2048 * 2, this, kAudioInputTaskPriority, &audio_input_task_handle_);
...
void AudioService::PushTaskToEncodeQueue(std::vector<int16_t>&& pcm) {
    auto task = std::make_unique<AudioTask>();
...
void AudioService::EnableWakeWordDetection(bool enable) {
    ...
    if (enable) {
        ...
        {
            std::lock_guard<std::mutex> lock(speaker_reference_resampler_mutex_);
            if (speaker_reference_resampler_ != nullptr) {
                esp_ae_rate_cvt_reset(speaker_reference_resampler_);
            }
        }
        wake_word_->Start();
        xEventGroupSetBits(event_group_, AS_EVENT_WAKE_WORD_RUNNING);
...
bool AudioService::EnableVoiceProcessing(bool enable) {
    ...
    if (enable) {
        if (!audio_processor_initialized_) {
            audio_processor_initialized_ = audio_processor_->Initialize(codec_, OPUS_FRAME_DURATION_MS, models_list_);
            if (!audio_processor_initialized_) {
...
bool AudioService::EnableDeviceAec(bool enable) {
    ...
    return audio_processor_->EnableDeviceAec(enable);
}

void AudioService::SetProcessedPcmCallback(std::function<void(std::vector<int16_t>&& data)> callback) {
    std::lock_guard<std::mutex> lock(processed_pcm_callback_mutex_);
...
void AudioService::PlaySound(const std::string_view& ogg) {
    if (audio_hardware_busy_) {
        ESP_LOGI(TAG, "Skip local sound playback because audio hardware is busy");
        return;
    }
...
void AudioService::SetAudioHardwareBusy(bool busy) {
    audio_hardware_busy_ = busy;
    if (busy) {
        last_input_time_ = std::chrono::steady_clock::now();
        last_output_time_ = std::chrono::steady_clock::now();
    }
...
void AudioService::HandleSpeakerReference(const std::vector<int16_t>& data) {
    if (codec_ == nullptr || audio_processor_ == nullptr || data.empty() || !audio_processor_->WantsSpeakerReference()) {
        return;
    }
    ...
    if (codec_->output_sample_rate() != 16000) {
        ...
        if (speaker_reference_resampler_ == nullptr) {
            esp_ae_rate_cvt_cfg_t resampler_cfg = RATE_CVT_CFG(
                codec_->output_sample_rate(), ESP_AUDIO_SAMPLE_RATE_16K, ESP_AUDIO_MONO);
            auto ret = esp_ae_rate_cvt_open(&resampler_cfg, &speaker_reference_resampler_);
            if (speaker_reference_resampler_ == nullptr) {
                ESP_LOGE(TAG, "Failed to create speaker reference resampler, error code: %d", ret);
                return;
            }
        }

        uint32_t output_samples = 0;
        esp_ae_rate_cvt_get_max_out_sample_num(speaker_reference_resampler_, data.size(), &output_samples);
        reference.resize(output_samples);
        uint32_t actual_output = output_samples;
        esp_ae_rate_cvt_process(speaker_reference_resampler_,
            reinterpret_cast<esp_ae_sample_t>(const_cast<int16_t*>(data.data())), data.size(),
            reinterpret_cast<esp_ae_sample_t>(reference.data()), &actual_output);
        reference.resize(actual_output);
        audio_processor_->FeedReference(std::move(reference));
    } else {
        audio_processor_->FeedReference(data.data(), data.size());
    }
}
