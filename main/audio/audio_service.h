...
#define AS_WAKE_WORD_ENABLED (CONFIG_USE_AFE_WAKE_WORD || CONFIG_USE_CUSTOM_WAKE_WORD || CONFIG_USE_ESP_WAKE_WORD)
...
#define AS_EVENT_WAKE_WORD_RUNNING          (1 << 0)
...
public:
    ...
    bool EnableDeviceAec(bool enable);
    void SetProcessedPcmCallback(std::function<void(std::vector<int16_t>&& data)> callback);
    ...
    void SetAudioHardwareBusy(bool busy);

private:
    void HandleSpeakerReference(const std::vector<int16_t>& data);

    AudioCodec* codec_ = nullptr;
    AudioServiceCallbacks callbacks_;
    std::mutex processed_pcm_callback_mutex_;
    std::function<void(std::vector<int16_t>&& data)> processed_pcm_callback_;
    ...
    std::mutex speaker_reference_resampler_mutex_;
    esp_ae_rate_cvt_handle_t input_resampler_ = nullptr;
    esp_ae_rate_cvt_handle_t output_resampler_ = nullptr;
    esp_ae_rate_cvt_handle_t speaker_reference_resampler_ = nullptr;
    ...
    bool audio_hardware_busy_ = false;
    ...
    void OpusCodecTask();
    void PushTaskToEncodeQueue(std::vector<int16_t>&& pcm);
