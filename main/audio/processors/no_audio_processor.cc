...
void NoAudioProcessor::Feed(std::vector<int16_t>&& data) {
    ...
    if (codec_->input_channels() > 1) {
        ...
        for (size_t i = 0, j = static_cast<size_t>(capture_channel); i < data.size() / codec_->input_channels(); ++i, j += codec_->input_channels()) {
...
bool NoAudioProcessor::EnableDeviceAec(bool enable) {
    if (enable) {
