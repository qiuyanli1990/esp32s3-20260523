...
#include "audio/reference_probe.h"
#endif
...
constexpr UBaseType_t kApplicationTaskPriority = 10;
constexpr size_t kScheduledTaskBatchLimit = 16;
constexpr EventBits_t kPriorityEventMask =
    ...
    MAIN_EVENT_AGENT_SELECTION_NEXT |
...
constexpr char kVoiceprintSampleUuidKey[] = "sample_uuid";
constexpr char kVoiceprintSampleBoardKey[] = "sample_board";
constexpr char kAgentSettingsNamespace[] = "wifi";
constexpr char kAgentIdKey[] = "agent_id";
constexpr int64_t kAgentSelectionIdleTimeoutMs = 5000;
constexpr int64_t kAgentSelectionPollMs = 100;
...
void AppendAgentOption(std::vector<AgentOption>& options, const std::string& raw_id,
    ...
    const auto it = std::find_if(options.begin(), options.end(), [&id](const AgentOption& option) {
        return option.id == id;
    });
    if (it != options.end()) {
        return;
    ...
    std::string label = Trim(raw_label);
    if (label.empty()) {
        label = id;
    }
...
std::string LoadSelectedAgentId() {
    Settings settings(kAgentSettingsNamespace, false);
    return Trim(settings.GetString(kAgentIdKey, SENTINO_DEFAULT_AGENT_ID));
}

void SaveSelectedAgentId(const std::string& agent_id) {
    std::string normalized_agent_id = Trim(agent_id);
    if (normalized_agent_id.empty()) {
        return;
    }

    Settings settings(kAgentSettingsNamespace, true);
    settings.SetString(kAgentIdKey, normalized_agent_id);
}

std::vector<AgentOption> LoadAgentOptions(const std::string& selected_agent_id) {
    std::vector<AgentOption> options;
    for (const auto& option : SENTINO_AGENT_OPTIONS) {
        AppendAgentOption(options, option.id, option.label);
    }

    const std::string default_agent_id = Trim(SENTINO_DEFAULT_AGENT_ID);
    if (!default_agent_id.empty()) {
        AppendAgentOption(options, default_agent_id, default_agent_id);
    }
    if (!selected_agent_id.empty()) {
        AppendAgentOption(options, selected_agent_id, selected_agent_id);
    }
    return options;
}

size_t FindAgentOptionIndex(const std::vector<AgentOption>& options, const std::string& selected_agent_id) {
    if (options.empty()) {
        return 0;
    }
    for (size_t i = 0; i < options.size(); ++i) {
        if (options[i].id == selected_agent_id) {
            return i;
        }
    }
    return 0;
...
std::string LoadVoiceprintSampleUrl() {
    ...
    std::string sample_url = Trim(settings.GetString(kVoiceprintSampleUrlKey, ""));
    if (sample_url.empty()) {
        return "";
    }

    std::string sample_uuid = Trim(settings.GetString(kVoiceprintSampleUuidKey, ""));
    std::string sample_board = Trim(settings.GetString(kVoiceprintSampleBoardKey, ""));
    std::string current_uuid = Board::GetInstance().GetUuid();
    if (sample_uuid != current_uuid || sample_board != BOARD_NAME) {
        ESP_LOGI(TAG, "Ignore cached voiceprint sample for board=%s uuid=%s while current board=%s uuid=%s",
            sample_board.empty() ? "<legacy>" : sample_board.c_str(),
            sample_uuid.empty() ? "<legacy>" : sample_uuid.c_str(),
            BOARD_NAME, current_uuid.c_str());
        return "";
    }
...
void SaveVoiceprintSampleUrl(const std::string& sample_url) {
    ...
    settings.SetString(kVoiceprintSampleUuidKey, Board::GetInstance().GetUuid());
    settings.SetString(kVoiceprintSampleBoardKey, BOARD_NAME);
}
...
void Application::Initialize() {
    ...
    realtime_audio_port_.SetCodec(audio_codec_);
...
#if CONFIG_AUDIO_REFERENCE_PROBE
    audio::RunReferenceProbe(audio_codec_);
#endif
...
void Application::Run() {
    vTaskPrioritySet(nullptr, kApplicationTaskPriority);
    ESP_LOGI(TAG, "Application main loop priority=%u",
        static_cast<unsigned>(kApplicationTaskPriority));
    ...
    while (true) {
        ...
        if (bits & MAIN_EVENT_AGENT_SELECTION_NEXT) {
            HandleAgentSelectionToggle();
        }
        ...
        if (bits & MAIN_EVENT_SCHEDULE) {
            ...
            while (true) {
                ...
                bool has_more_tasks = false;
                {
                    ...
                    has_more_tasks = !main_tasks_.empty();
                }
                task();
                ++processed;

                EventBits_t pending = xEventGroupGetBits(event_group_);
                if ((pending & kPriorityEventMask) || processed >= kScheduledTaskBatchLimit) {
                    if (has_more_tasks) {
...
bool Application::CycleAgentSelection() {
    if (!agent_selection_active_) {
        return false;
    }
    xEventGroupSetBits(event_group_, MAIN_EVENT_AGENT_SELECTION_NEXT);
    return true;
}
...
void Application::ShowAgentSelectionOption(const AgentSelectionOption& option, bool selected) {
    auto display = Board::GetInstance().GetDisplay();
    std::string label = option.label.empty() ? option.id : option.label;
    if (selected) {
        std::string message = "Agent selected: " + label;
        ESP_LOGI(TAG, "Agent selection confirmed: label=%s id=%s", label.c_str(), option.id.c_str());
        if (display != nullptr) {
            display->SetStatus(Lang::Strings::CONNECTING);
            display->SetChatMessage("subtitle", "");
            display->ShowNotification(message.c_str(), 1500);
        }
        return;
    }

    std::string notification = "Agent: " + label;
    ESP_LOGI(TAG, "Agent selection option: label=%s id=%s", label.c_str(), option.id.c_str());
    if (display != nullptr) {
        display->SetStatus(notification.c_str());
        display->SetChatMessage("subtitle", "");
        display->ShowNotification(notification.c_str(), 1500);
    }
}

bool Application::HandleAgentSelectionToggle() {
    if (!agent_selection_active_) {
        return false;
    }

    AgentSelectionOption option;
    {
        std::lock_guard<std::mutex> lock(agent_selection_mutex_);
        if (agent_selection_options_.empty()) {
            return true;
        }
        agent_selection_index_ = (agent_selection_index_ + 1) % agent_selection_options_.size();
        agent_selection_deadline_ms_ = NowMs() + kAgentSelectionIdleTimeoutMs;
        option = agent_selection_options_[agent_selection_index_];
    }

    ESP_LOGI(TAG, "Agent selection changed: %s", option.id.c_str());
    ShowAgentSelectionOption(option, false);
    return true;
}

bool Application::SelectAgentBeforeConversationStart() {
    const std::string selected_agent_id = LoadSelectedAgentId();
    const auto loaded_options = LoadAgentOptions(selected_agent_id);
    if (loaded_options.empty()) {
        ESP_LOGW(TAG, "No Sentino agent options configured; falling back to default resolution");
        return true;
    }

    if (loaded_options.size() == 1) {
        SaveSelectedAgentId(loaded_options.front().id);
        ESP_LOGI(TAG, "Use only configured Sentino agent: %s", loaded_options.front().id.c_str());
        return true;
    }

    std::vector<AgentSelectionOption> options;
    options.reserve(loaded_options.size());
    for (const auto& option : loaded_options) {
        options.push_back({option.id, option.label});
    }

    const size_t initial_index = FindAgentOptionIndex(loaded_options, selected_agent_id);
    AgentSelectionOption initial_option = options[initial_index];
    auto display = Board::GetInstance().GetDisplay();
    if (display == nullptr) {
        SaveSelectedAgentId(initial_option.id);
        ESP_LOGI(TAG, "Use selected Sentino agent without display prompt: %s", initial_option.id.c_str());
        return true;
    }

    {
        std::lock_guard<std::mutex> lock(agent_selection_mutex_);
        agent_selection_options_ = options;
        agent_selection_index_ = initial_index;
        agent_selection_deadline_ms_ = NowMs() + kAgentSelectionIdleTimeoutMs;
        agent_selection_active_ = true;
    }

    ESP_LOGI(TAG, "Agent selection window opened with %u options, initial=%s",
        static_cast<unsigned>(options.size()), initial_option.id.c_str());
    Schedule([this, initial_option]() {
        ShowAgentSelectionOption(initial_option, false);
    });
    ...
    while (true) {
        if (!network_connected_) {
            {
                std::lock_guard<std::mutex> lock(agent_selection_mutex_);
                agent_selection_active_ = false;
                agent_selection_options_.clear();
            }
            SetError("Network is not connected");
            return false;
        }

        AgentSelectionOption final_option;
        bool timed_out = false;
        {
            std::lock_guard<std::mutex> lock(agent_selection_mutex_);
            if (!agent_selection_active_ || agent_selection_options_.empty()) {
                return false;
            }
            if (NowMs() >= agent_selection_deadline_ms_) {
                final_option = agent_selection_options_[agent_selection_index_];
                agent_selection_active_ = false;
                agent_selection_options_.clear();
                timed_out = true;
            }
        ...
        if (timed_out) {
            SaveSelectedAgentId(final_option.id);
            ESP_LOGI(TAG, "Agent selected before conversation start: %s", final_option.id.c_str());
            Schedule([this, final_option]() {
                ShowAgentSelectionOption(final_option, true);
            });
            return true;
        }

        vTaskDelay(pdMS_TO_TICKS(kAgentSelectionPollMs));
    }
}
...
bool Application::StartAgoraAgentInternal() {
    ...
    if (VoiceprintUploader::IsEnabled()) {
        ESP_LOGI(TAG, "Voiceprint enabled, checking cached sample");
        ...
        if (!last_voiceprint_sample_url_.empty()) {
            ...
            ESP_LOGI(TAG, "No voiceprint sample cache for current board and UUID, start capture and upload");
        ...
        ESP_LOGI(TAG, "Voiceprint skipped because voiceprint enrollment is disabled");
    }

    if (!SelectAgentBeforeConversationStart()) {
        return false;
    }
...
bool Application::StartAgoraSessionInternal() {
    ...
    ESP_LOGI(TAG, "Conversation session info: channel=%s conversation_id=%s join_uid=%u",
    ...
    if (aec_mode_ == kAecOnDeviceSide && audio_codec_ != nullptr && audio_codec_->supports_device_aec()) {
        if (!audio_codec_->has_hardware_input_reference()) {
            audio_codec_->SetReferenceOrder(AudioReferenceOrder::kReferenceAfterNearEnd);
    ...
    agora_rtc_session_.SetStreamMessageCallback([this](uint32_t uid, const std::string& payload) {
        ESP_LOGD(TAG, "Queue datastream payload for UI: uid=%lu payload=%s",
            static_cast<unsigned long>(uid), payload.c_str());
        std::string payload_copy = payload;
        Schedule([uid, payload = std::move(payload_copy)]() {
            ESP_LOGD(TAG, "Datastream event on main loop: uid=%lu payload=%s",
                static_cast<unsigned long>(uid), payload.c_str());
            auto display = Board::GetInstance().GetDisplay();
            DatastreamDisplayActions actions;
            ...
            if (displayed) {
                if (actions.has_emotion_action) {
                    ESP_LOGI(TAG, "Weather action displayed; skipping same-payload emotion action=%s emotion=%s",
                        actions.emotion_action_type.c_str(), actions.emotion.c_str());
                }
                return;
            }
#endif
    ...
    if (use_device_aec) {
        ESP_LOGI(TAG, "Preparing device-side AEC before Agora playback starts");
        if (audio_codec_ != nullptr && !audio_codec_->input_enabled()) {
            ESP_LOGI(TAG, "Opening input before AEC warmup to avoid input/output I2S reconfigure race");
            audio_codec_->EnableInput(true);
        }
        audio_service_.SetProcessedPcmCallback([](std::vector<int16_t>&& data) {
            (void)data;
        });
    ...
    if (!agora_audio_bridge_.Start(&realtime_audio_port_, &audio_service_, &agora_rtc_session_,
