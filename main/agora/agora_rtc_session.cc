...
bool AgoraRtcSession::Start(const AgoraAgentConfig& config) {
    ...
    ESP_LOGI(kTag,
        "Join RTC: channel=%s uid=%lu codec=%d sample_rate=%d channels=%d duration=%d sub_audio=%d sub_video=%d",
...
void AgoraRtcSession::HandleStreamChunk(uint32_t uid, const char* data, size_t length) {
    ...
    if (message_user_chunk) {
        ESP_LOGI(kTag, "Raw message.user chunk uid=%lu len=%u preview=%s",
...
void AgoraRtcSession::OnStreamMessage(connection_id_t conn_id, uint32_t uid, int stream_id, const char* data,
    ...
    std::string preview = SummarizeStreamPreview(data, length);
    std::string object_name = ExtractObjectNameForLog(preview);
    if (object_name == "message.user") {
        ESP_LOGI(kTag, "Raw stream uid=%lu stream_id=%d object=%s len=%u preview=%s",
            static_cast<unsigned long>(uid), stream_id, object_name.c_str(),
            static_cast<unsigned>(length), ShortenForLog(preview).c_str());
    }
