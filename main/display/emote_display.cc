...
constexpr int kEmoteFps = 30;
constexpr int kEmoteTaskPriority = 5;
constexpr bool kEmoteBufferDma = false;

#define EMOTE_LOG_UI(...) ESP_LOGI(TAG, __VA_ARGS__)
...
static emote_handle_t InitializeEmote(const esp_lcd_panel_handle_t panel, const int width, const int height)
{
    ...
    emote_config_t emote_cfg = {
        ...
        .task = {
            .task_priority = kEmoteTaskPriority,
            .task_stack = 6 * 1024,
            .task_affinity = 0,
...
{
    const char* safe_emotion = SafeText(emotion);
    EMOTE_LOG_UI("SetEmotion: %s", safe_emotion);
    StopWeatherTimer();
...
{
    const char* safe_role = SafeText(role);
    const char* safe_content = SafeText(content);
    EMOTE_LOG_UI("SetChatMessage: %s, %s", safe_role, safe_content);
    StopWeatherTimer();
    HideWeatherSideLabels();
    StopAnimDialog();
    if (emote_handle_ && content && strlen(content) > 0) {
        if (std::strcmp(safe_role, "system") == 0) {
...
{
    const char* safe_status = SafeText(status);
    EMOTE_LOG_UI("SetStatus: %s", safe_status);
    StopWeatherTimer();
    HideWeatherSideLabels();
    StopAnimDialog();
...
{
    (void)duration_ms;
    const char* safe_notification = SafeText(notification);
    EMOTE_LOG_UI("ShowNotification: %s", safe_notification);
    StopWeatherTimer();
    HideWeatherSideLabels();
    StopAnimDialog();
    ...
    const char* city, const char* temperature, int duration_ms)
{
    ...
    EMOTE_LOG_UI("ShowWeather: condition=%s summary=%s city=%s temperature=%s duration_ms=%d",
        condition ? condition : "",
        summary ? summary : "",
        city ? city : "",
...
{
    EMOTE_LOG_UI("SetPowerSaveMode: %s", on ? "ON" : "OFF");
    if (!emote_handle_) {
        return;
    }
}
...
{
    if (image) {
        EMOTE_LOG_UI("SetPreviewImage: Preview image not supported, using default icon");
    }
}

void EmoteDisplay::SetTheme(Theme* const theme)
{
    EMOTE_LOG_UI("SetTheme: %p", theme);
}
...
{
    EMOTE_LOG_UI("StopAnimDialog");
    if (emote_handle_) {
        bool stopped = emote_stop_anim_dialog(emote_handle_) == ESP_OK;
        return stopped;
    }
    return false;
}
...
{
    EMOTE_LOG_UI("InsertAnimDialog: %s, %" PRIu32, emoji_name, duration_ms);
    if (emote_handle_ && emoji_name) {
        bool inserted = emote_insert_anim_dialog(emote_handle_, emoji_name, duration_ms) == ESP_OK;
        return inserted;
    }
    return false;
}
