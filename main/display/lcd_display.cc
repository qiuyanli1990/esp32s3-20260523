...
static const lv_font_t* SelectWeatherLabelFont(const lv_font_t* text_font, int screen_width) {
#if CONFIG_BOARD_TYPE_SEEED_STUDIO_SENSECAP_WATCHER
    if (screen_width >= 360) {
        return &font_noto_basic_30_4;
    }
#endif
    return text_font != nullptr && text_font->line_height > font_noto_basic_20_4.line_height
        ? &font_noto_basic_20_4
        : text_font;
}

#if CONFIG_BOARD_TYPE_SEEED_STUDIO_SENSECAP_WATCHER
constexpr uint32_t kLcdEmotionScalePercent = 38;
#else
constexpr uint32_t kLcdEmotionScalePercent = 100;
#endif

void LcdDisplay::ShowWeather(const char* condition, const char* summary,
    const char* city, const char* temperature, int duration_ms) {
    if (summary != nullptr && summary[0] != '\0') {
        ShowNotification(summary, duration_ms);
    }
    ...
    if (preview_image_ == nullptr || emoji_box_ == nullptr || preview_timer_ == nullptr) {
        return;
    }

    std::string asset_name = "weather-";
    ...
    if (!Assets::GetInstance().GetAssetData(asset_name, ptr, size)) {
        ESP_LOGW(TAG, "Weather asset not found: %s", asset_name.c_str());
        return;
    }

    DisplayLockGuard lock(this);
    preview_image_cached_ = std::make_unique<LvglRawImage>(ptr, size);
    ...
    ESP_ERROR_CHECK(esp_timer_start_once(preview_timer_, std::max(duration_ms, 1) * 1000));
}
...
void LcdDisplay::InitializeLcdThemes() {
    ...
    std::shared_ptr<EmojiCollection> fallback_emoji_collection;
    if (width_ >= 240) {
        fallback_emoji_collection = std::make_shared<Twemoji64>();
    } else {
        fallback_emoji_collection = std::make_shared<Twemoji32>();
    }
    ...
    light_theme->set_emoji_collection(fallback_emoji_collection);
    ...
    dark_theme->set_emoji_collection(fallback_emoji_collection);
...
void LcdDisplay::ApplyEmotionImageScale(const lv_image_dsc_t* image_dsc) {
    if (emoji_box_ == nullptr || emoji_image_ == nullptr || image_dsc == nullptr ||
        image_dsc->header.w == 0 || image_dsc->header.h == 0) {
        return;
    }
    ...
    const uint32_t scale_x = (static_cast<uint32_t>(width_) * 256U + image_dsc->header.w - 1) / image_dsc->header.w;
    const uint32_t scale_y = (static_cast<uint32_t>(height_) * 256U + image_dsc->header.h - 1) / image_dsc->header.h;
    uint32_t scale = std::max(scale_x, scale_y) * kLcdEmotionScalePercent / 100U;
    lv_image_set_scale(emoji_image_, std::max<uint32_t>(scale, 1));
}

void LcdDisplay::ShowWeatherSideLabels(const char* city, const char* temperature) {
    ...
    if (weather_city_label_ == nullptr) {
        weather_city_label_ = lv_label_create(screen);
    }
    if (weather_temperature_label_ == nullptr) {
        weather_temperature_label_ = lv_label_create(screen);
    }
    ...
    auto text_font = lvgl_theme != nullptr && lvgl_theme->text_font() != nullptr
        ? lvgl_theme->text_font()->font()
        : nullptr;
    lv_color_t text_color = lvgl_theme != nullptr ? lvgl_theme->text_color() : lv_color_white();

    int screen_width = width_ > 0 ? width_ : LV_HOR_RES;
    int screen_height = height_ > 0 ? height_ : LV_VER_RES;
    auto weather_font = SelectWeatherLabelFont(text_font, screen_width);
    int preview_width = std::max(80, screen_width / 2);
    int safety_margin = std::clamp(screen_width / 12, 12, 32);
    int image_gap = std::max(4, screen_width / 56);
    int inward_shift = std::clamp(screen_width / 24, 6, 12);
    int label_width = std::max(48,
        (screen_width - preview_width) / 2 - safety_margin - image_gap + inward_shift + 8);
#if CONFIG_BOARD_TYPE_SEEED_STUDIO_SENSECAP_WATCHER
    if (screen_width >= 360) {
        label_width = std::max(label_width, std::clamp(screen_width / 3, 118, 140));
    }
#endif
    int max_label_width = screen_width >= 480 ? 180 : 108;
#if CONFIG_BOARD_TYPE_SEEED_STUDIO_SENSECAP_WATCHER
    if (screen_width >= 360) {
        max_label_width = 140;
    }
    ...
    int label_height = weather_font != nullptr ? weather_font->line_height : (screen_height >= 360 ? 28 : 22);
    int x_offset = screen_width / 2 - safety_margin - label_width / 2 - inward_shift;
#if CONFIG_BOARD_TYPE_SEEED_STUDIO_SENSECAP_WATCHER
    if (screen_width >= 360) {
        x_offset = std::max(x_offset, screen_width / 4);
        x_offset += std::clamp(screen_width / 34, 10, 14);
    }
#endif
    int y_offset = screen_height >= 360 ? 20 : 12;

    if (weather_city_label_ != nullptr) {
        lv_obj_set_size(weather_city_label_, label_width, label_height);
        lv_label_set_long_mode(weather_city_label_, LV_LABEL_LONG_CLIP);
        lv_obj_set_style_text_align(weather_city_label_, LV_TEXT_ALIGN_RIGHT, 0);
        lv_obj_set_style_text_color(weather_city_label_, text_color, 0);
        if (weather_font != nullptr) {
            lv_obj_set_style_text_font(weather_city_label_, weather_font, 0);
        }
        lv_label_set_text(weather_city_label_, has_city ? city : "");
        lv_obj_align(weather_city_label_, LV_ALIGN_CENTER, -x_offset, y_offset);
        lv_obj_move_to_index(weather_city_label_, -1);
        if (has_city) {
            lv_obj_remove_flag(weather_city_label_, LV_OBJ_FLAG_HIDDEN);
        } else {
            lv_obj_add_flag(weather_city_label_, LV_OBJ_FLAG_HIDDEN);
        }
    }

    if (weather_temperature_label_ != nullptr) {
        lv_obj_set_size(weather_temperature_label_, label_width, label_height);
        lv_label_set_long_mode(weather_temperature_label_, LV_LABEL_LONG_CLIP);
        lv_obj_set_style_text_align(weather_temperature_label_, LV_TEXT_ALIGN_LEFT, 0);
        lv_obj_set_style_text_color(weather_temperature_label_, text_color, 0);
        if (weather_font != nullptr) {
            lv_obj_set_style_text_font(weather_temperature_label_, weather_font, 0);
        }
        lv_label_set_text(weather_temperature_label_, has_temperature ? temperature : "");
        lv_obj_align(weather_temperature_label_, LV_ALIGN_CENTER, x_offset, y_offset);
        lv_obj_move_to_index(weather_temperature_label_, -1);
        if (has_temperature) {
            lv_obj_remove_flag(weather_temperature_label_, LV_OBJ_FLAG_HIDDEN);
        } else {
            lv_obj_add_flag(weather_temperature_label_, LV_OBJ_FLAG_HIDDEN);
        }
    }
}

void LcdDisplay::HideWeatherSideLabels() {
    if (weather_city_label_ != nullptr) {
...
LcdDisplay::~LcdDisplay() {
    ...
    if (weather_city_label_ != nullptr) {
        lv_obj_del(weather_city_label_);
    }
    if (weather_temperature_label_ != nullptr) {
        lv_obj_del(weather_temperature_label_);
    }
    if (chat_message_label_ != nullptr) {
        lv_obj_del(chat_message_label_);
    }
    if (subtitle_overlay_ != nullptr) {
        lv_obj_del(subtitle_overlay_);
        // subtitle_label_ is automatically deleted as a child of subtitle_overlay_
    }
...
void LcdDisplay::SetupUI() {
    ...
    // Initialize subtitle overlay (displays complete sentences with final=true)
    subtitle_overlay_ = lv_obj_create(screen);
    lv_obj_set_width(subtitle_overlay_, LV_HOR_RES);           // Full screen width
    lv_obj_set_height(subtitle_overlay_, LV_SIZE_CONTENT);     // Content-based height
    lv_obj_set_style_radius(subtitle_overlay_, 0, 0);          // No rounded corners
    lv_obj_set_style_bg_color(subtitle_overlay_, lvgl_theme->background_color(), 0);
    lv_obj_set_style_bg_opa(subtitle_overlay_, LV_OPA_50, 0);  // 50% transparency
    lv_obj_set_style_border_width(subtitle_overlay_, 0, 0);    // No border
    lv_obj_set_style_pad_all(subtitle_overlay_, lvgl_theme->spacing(4), 0);  // Padding
    lv_obj_set_scrollbar_mode(subtitle_overlay_, LV_SCROLLBAR_MODE_OFF);
    lv_obj_align(subtitle_overlay_, LV_ALIGN_BOTTOM_MID, 0, 0);  // Bottom center position
    lv_obj_add_flag(subtitle_overlay_, LV_OBJ_FLAG_HIDDEN);    // Initially hidden

    // Create subtitle label inside overlay
    subtitle_label_ = lv_label_create(subtitle_overlay_);
    lv_label_set_text(subtitle_label_, "");
    lv_obj_set_width(subtitle_label_, lv_pct(100));            // 100% of parent width
    lv_label_set_long_mode(subtitle_label_, LV_LABEL_LONG_WRAP);  // Enable text wrapping
    lv_obj_set_style_text_align(subtitle_label_, LV_TEXT_ALIGN_LEFT, 0);
    lv_obj_set_style_text_color(subtitle_label_, lvgl_theme->text_color(), 0);
...
void LcdDisplay::SetChatMessage(const char* role, const char* content) {
    ...
    // Handle subtitle display (only shows complete sentences with final=true)
    if (role != nullptr && strcmp(role, "subtitle") == 0) {
        if (subtitle_label_ == nullptr || subtitle_overlay_ == nullptr) {
            ESP_LOGW(TAG, "SetChatMessage subtitle failed: subtitle components not initialized");
            return;
        }
        lv_label_set_text(subtitle_label_, content ? content : "");
        if (hide_subtitle_ || content == nullptr || content[0] == '\0') {
            lv_obj_add_flag(subtitle_overlay_, LV_OBJ_FLAG_HIDDEN);
        } else {
            lv_obj_remove_flag(subtitle_overlay_, LV_OBJ_FLAG_HIDDEN);
            // Position is set only once in SetupUI, no need to realign on each update
        }
        return;
    }
    ...
    if (image->IsGif()) {
        ...
        if (gif_controller_->IsLoaded()) {
            ...
            gif_controller_->SetFrameCallback([this]() {
                auto frame = gif_controller_ ? gif_controller_->image_dsc() : nullptr;
                if (emoji_image_ == nullptr || frame == nullptr) {
                    return;
                }
                lv_image_cache_drop(frame);
                lv_obj_invalidate(emoji_image_);
            });
            
            // Set initial frame and start animation
            lv_image_set_src(emoji_image_, gif_controller_->image_dsc());
            ApplyEmotionImageScale(gif_controller_->image_dsc());
            gif_controller_->Start();
        ...
        ApplyEmotionImageScale(image->image_dsc());
        lv_obj_add_flag(emoji_label_, LV_OBJ_FLAG_HIDDEN);
        lv_obj_remove_flag(emoji_image_, LV_OBJ_FLAG_HIDDEN);
    }
...
void LcdDisplay::SetTheme(Theme* theme) {
    ...
    auto icon_font = lvgl_theme->icon_font()->font();
    auto large_icon_font = lvgl_theme->large_icon_font()->font();
    auto weather_font = SelectWeatherLabelFont(text_font, width_ > 0 ? width_ : LV_HOR_RES);
    ...
    if (weather_city_label_ != nullptr) {
        lv_obj_set_style_text_font(weather_city_label_, weather_font, 0);
        lv_obj_set_style_text_color(weather_city_label_, lvgl_theme->text_color(), 0);
    }
    if (weather_temperature_label_ != nullptr) {
        lv_obj_set_style_text_font(weather_temperature_label_, weather_font, 0);
        lv_obj_set_style_text_color(weather_temperature_label_, lvgl_theme->text_color(), 0);
    }
...
void LcdDisplay::SetHideSubtitle(bool hide) {
    ...
    // Handle subtitle overlay visibility
    if (subtitle_overlay_ != nullptr) {
        if (hide) {
            lv_obj_add_flag(subtitle_overlay_, LV_OBJ_FLAG_HIDDEN);
        } else {
            // Only show if there is actual subtitle content
            const char* text = (subtitle_label_ != nullptr) ? lv_label_get_text(subtitle_label_) : nullptr;
            if (text != nullptr && text[0] != '\0') {
                lv_obj_remove_flag(subtitle_overlay_, LV_OBJ_FLAG_HIDDEN);
            }
        }
