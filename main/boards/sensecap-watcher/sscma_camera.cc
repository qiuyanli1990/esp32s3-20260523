...
SscmaCamera::SscmaCamera(esp_io_expander_handle_t io_exp_handle) {
    ...
    xTaskCreate([](void* arg) {
        ...
        while (true)
        {
            ...
            if (!this_->session_active_ && this_->inference_en && Application::GetInstance().GetDeviceState() == kDeviceStateIdle ) {
                if (!is_inference) {
            ...
            } else if (is_inference && (this_->session_active_ || !this_->inference_en || Application::GetInstance().GetDeviceState() != kDeviceStateIdle))  {
...
void SscmaCamera::SetSessionActive(bool active) {
    session_active_ = active;
}

bool SscmaCamera::StartVideoStream(VideoStreamCallback callback) {
    if (sscma_client_handle_ == nullptr || callback == nullptr) {
        return false;
    }
    if (video_streaming_) {
        video_stream_callback_ = std::move(callback);
        return true;
    }
    ...
    xTaskCreate([](void* arg) {
        auto* self = static_cast<SscmaCamera*>(arg);
        while (self->video_streaming_) {
            if (sscma_client_set_sensor(self->sscma_client_handle_, 1, 3, true) != ESP_OK) {
                vTaskDelay(pdMS_TO_TICKS(200));
                continue;
            }
            if (sscma_client_sample(self->sscma_client_handle_, 1) != ESP_OK) {
                vTaskDelay(pdMS_TO_TICKS(100));
                continue;
            }

            SscmaData data{};
            if (xQueueReceive(self->sscma_data_queue_, &data, pdMS_TO_TICKS(1200)) != pdPASS) {
                continue;
            }

            size_t jpeg_len = 0;
            int ret = mbedtls_base64_decode(self->jpeg_data_.buf, IMG_JPEG_BUF_SIZE, &jpeg_len, data.img, data.len);
            heap_caps_free(data.img);
            if (ret != 0 || jpeg_len == 0) {
                continue;
            }
            self->jpeg_data_.len = jpeg_len;

            if (self->video_stream_callback_) {
                CameraVideoFrame frame;
                frame.data = self->jpeg_data_.buf;
                frame.len = self->jpeg_data_.len;
                frame.format = CameraStreamFormat::kJpeg;
                frame.key_frame = true;
                frame.frame_rate = 1;
                self->video_stream_callback_(frame);
            }

            vTaskDelay(pdMS_TO_TICKS(500));
        }

        self->video_stream_task_handle_ = nullptr;
        vTaskDelete(nullptr);
    }, "sscma_video_stream", 6144, this, 3, &video_stream_task_handle_);

    return true;
}

void SscmaCamera::StopVideoStream() {
