...
def process_sr_models(model_dirs, build_dir, assets_dir):
    ...
    for model_dir in model_dirs:
        ...
        if copy_directory(model_dir, model_dst):
            models_processed += 1
            print(f"Added SR model: {model_name}")
    ...
    for extra_files_dir in extra_files_dirs:
        ...
        for root, dirs, files in os.walk(extra_files_dir):
            for file in files:
                ...
                dst_file = os.path.join(assets_dir, file)
                if copy_file(src_file, dst_file):
                    if file not in extra_files_list:
...
def read_nsnet_from_sdkconfig(sdkconfig_path):
    """Read selected NSNET models from sdkconfig."""
    if not os.path.exists(sdkconfig_path):
        print(f"Warning: sdkconfig file not found: {sdkconfig_path}")
        return []
    ...
    with io.open(sdkconfig_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if line.startswith('#'):
                continue
            if 'CONFIG_SR_NSN_NSNET2=y' in line:
...
def read_vadnet_from_sdkconfig(sdkconfig_path):
    """Read selected VADNET models from sdkconfig."""
    if not os.path.exists(sdkconfig_path):
        print(f"Warning: sdkconfig file not found: {sdkconfig_path}")
        return []
    ...
    with io.open(sdkconfig_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if line.startswith('#'):
                continue
            if 'CONFIG_SR_VADN_VADNET1_MEDIUM=y' in line:
...
def get_model_paths(model_names, model_subdir, esp_sr_model_path):
    """Get the full paths to model directories under the given subdirectory."""
    if not model_names:
        return []
    
    valid_paths = []
    for model_name in model_names:
        model_path = os.path.join(esp_sr_model_path, model_subdir, model_name)
...
def get_wakenet_model_paths(model_names, esp_sr_model_path):
    return get_model_paths(model_names, 'wakenet_model', esp_sr_model_path)


def get_multinet_model_paths(model_names, esp_sr_model_path):
    return get_model_paths(model_names, 'multinet_model', esp_sr_model_path)


def get_nsnet_model_paths(model_names, esp_sr_model_path):
    return get_model_paths(model_names, 'nsnet_model', esp_sr_model_path)


def get_vadnet_model_paths(model_names, esp_sr_model_path):
    return get_model_paths(model_names, 'vadnet_model', esp_sr_model_path)
...
def build_assets_integrated(sr_model_paths, text_font_path, emoji_collection_path, extra_files_path, output_path, multinet_model_info=None):
    """
    ...
    try:
        ...
        srmodels = process_sr_models(sr_model_paths, temp_build_dir, assets_dir) if sr_model_paths else None
...
def main():
    ...
    parser.add_argument('--extra_files', action='append',
                        help='Path to an extra files directory to be included in assets (can be passed multiple times)')
    ...
    sr_model_paths = wakenet_model_paths + multinet_model_paths + nsnet_model_paths + vadnet_model_paths
    ...
    if not sr_model_paths and not text_font_path and not emoji_collection_path and not extra_files_path and not multinet_model_info:
    ...
    # Build the assets
    success = build_assets_integrated(sr_model_paths, text_font_path, emoji_collection_path, 
