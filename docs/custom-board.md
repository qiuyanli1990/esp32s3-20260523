...
- `config.json` - 编译元数据，指定目标芯片、变体名称以及可选的兼容配置
- `sdkconfig.defaults` - 板级默认构建配置，推荐放 flash、partition、FreeRTOS、AEC 等系统级选项
...
- `sdkconfig_append`: 可选的额外兼容配置。推荐只放“变体特有”的少量覆盖项；不要再把整块板子的系统配置都堆在这里。

#### sdkconfig.defaults

推荐把板级系统配置写到`main/boards/<board>/sdkconfig.defaults`里。`release.py` 会把它和项目根目录的 `sdkconfig.defaults`、`sdkconfig.defaults.<target>` 合并，生成每个变体独立的 `.sdkconfig/<variant>.sdkconfig`，再在隔离的 `.build/<variant>` 目录里执行构建，避免切板时互相污染。

开发调试时，也可以运行 `python scripts/select_board.py <board>`。它会复用同一套板级 defaults 逻辑，把合并后的结果写到根目录 `sdkconfig`，并把根 `build/` 切到当前开发板。这样切板一次后，就可以继续使用标准的：
...
之所以不用“把所有板级项直接丢给 `SDKCONFIG_DEFAULTS`”这条纯 defaults 流，是因为 ESP-IDF 对 `choice symbol` 的 defaults 支持不完整，像 `flash size`、`partition table` 这类选项会失效。现在这套“板级 defaults -> 生成独立 sdkconfig”更稳。
...
- `config.h`: GPIO、屏幕尺寸、codec 能力、采样率等硬件能力
- `sdkconfig.defaults`: flash、partition、bootloader、FreeRTOS、系统功能开关的板级种子配置
- `config.json -> sdkconfig_append`: 少量变体差异或尚未迁移到 `sdkconfig.defaults` 的兼容项；`release.py` 会把它们并进最终 `.sdkconfig/<variant>.sdkconfig`
...
2. **初始化函数**：包括I2C、显示屏、按钮、音频等组件的初始化
...
4. **无法与服务器通信**：检查网络连接、`SENTINO_API_BEARER_TOKEN` 和会话参数配置
